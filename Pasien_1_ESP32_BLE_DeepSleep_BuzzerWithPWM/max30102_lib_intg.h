 ...
 */
#ifndef _MAX30102_LIB_INTG_H
#define _MAX30102_LIB_INTG_H
...
class MAX30102_LIB_INTG
{
  ...
  */
   ...
   */
  void sensorConfiguration(uint8_t ledBrightness = 60, uint8_t sampleAverage = SAMPLEAVG_8, \
                           uint8_t ledMode = MODE_MULTILED, uint8_t sampleRate = SAMPLERATE_400, \
                           uint8_t pulseWidth = PULSEWIDTH_411, uint8_t adcRange = ADCRANGE_16384);
   ...
   *@param *ratio                 [out]Calculated HR autocorrelation ratio
   *@param *correl                [out]Calculated Pearson correlation
   */
  void heartrateAndOxygenSaturation(double* SPO2,int8_t* SPO2Valid,int32_t* heartRate,int8_t* heartRateValid, double* ratio, double* correl);
